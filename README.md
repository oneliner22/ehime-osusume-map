...
## 閲覧側の機能（index.html、すべてブラウザ内で完結）

- **ブックマーク**: 一覧タイル・詳細モーダル・ランダム枠の ☆ で追加/解除。`localStorage`（キー `spotmap:bm:<パス>`）にだけ保存し、
  ログイン不要・サーバ側には何も持たない（ブラウザのデータ削除で消える）。
  - 「ブックマーク＝いずれかのグループに所属」。☆ は「★の追加先」グループに入れる／全グループから外す。
    モーダルのグループチップで所属を個別に切り替えられる。
  - グループは最大 10 個・各 100 件。名前変更（✎）・並べ替え（▲▼）・削除（🗑 を2回）。スポットも ▲▼ で並べ替え。
  - 🧭 動線: グループのスポットを表示順に線で結び、番号付きマーカーを地図に描く（day1/day2 の動線確認用）。グループごとに色が違う。
  - 「⭐ ブックマークのみ」チップで地図と一覧をブックマーク済みに絞れる（ジャンル・エリアとは独立）。
- **絞り込み・並び順**（地図の下）: 出典の種類（YouTube / X）、おすすめした人（Xの投稿者・YouTubeチャンネル、件数順）、
  並び順「指定した場所から近い順」（起点は 現在地 / いまの地図の中心 / スポット名 から選ぶ）。追加データは不要で、すべて `spots.json` と `videos.json` から実行時に組み立てる。
- **新着スポット**: 冒頭に `added`（追加日）の新しい順で 5 件（`config.reco_count` で変更可）を横スクロールのカルーセルで表示。
  「次の5件を見る」で 5 件ずつページ送りし、末尾まで行くと「最初に戻る」。カードに「📅 9/12 追加」を表示し、
  `added` のない初期収録分（YouTube 由来の 152 件、2026-09-05 公開）は「初期収録」と表示して末尾に並ぶ。
- **ランダムおすすめ**: 新着の下に 5 件を同じカルーセルで表示。「別の5件を見る」で引き直す。
  どちらも地図が押し出されないよう高さ固定で、「隠す」の状態は `localStorage` に個別に記憶する。
