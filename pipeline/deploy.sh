...
# 愛媛版: Secret (github-token / xdev-mcp-url / places-api-key) は ishikawa-spots と共用。
#   既に存在すれば SKIP_SECRETS=1 で upsert を飛ばせる
set -euo pipefail

PROJECT=central-bulwark-427114-j7
REGION=asia-northeast1
JOB=ehime-spots-daily
SA_NAME=ehime-spots-crj
SA="${SA_NAME}@${PROJECT}.iam.gserviceaccount.com"
IMAGE="${REGION}-docker.pkg.dev/${PROJECT}/ehime-spots/daily:latest"
...
gcloud artifacts repositories describe ehime-spots --location="$REGION" >/dev/null 2>&1 || \
  gcloud artifacts repositories create ehime-spots --repository-format=docker --location="$REGION"

gcloud iam service-accounts describe "$SA" >/dev/null 2>&1 || \
  gcloud iam service-accounts create "$SA_NAME" --display-name="ehime-spots daily ingest"
...
if ! gcloud secrets describe places-api-key >/dev/null 2>&1; then
  KEY=$(gcloud services api-keys create --display-name="ehime-spots-places" \
...
SCHED=ehime-spots-daily-trigger
