...
必要な環境変数:
  GITHUB_REPO   owner/name (例 oneliner22/ehime-osusume-map)
...
def watch_official(mcp, ledger, rh):
    ...
    for p in posts:
        j = gen_json(MODEL_EXTRACT, ASK_SCHEMA, [
            ...
            "愛媛公演「ぽこピーの回覧板」(2026年10月12日・松山市民会館)に関連して、ファンにおすすめスポット・グルメ等の"
...
CONTINUATION_NOTE = ("(注: この投稿は、#ぽこピーの回覧板 向け愛媛おすすめ紹介"
...
def extract_spots(post, images):
    parts = [
        "以下はX(旧Twitter)の投稿です。「#ぽこピーの回覧板」愛媛公演(松山・2026年10月12日)に向けた"
        "愛媛県・四国のおすすめスポット紹介かどうかを判定し、紹介されている具体的な"
        "(注意: 既に終わった石川公演(金沢)の感想・石川のおすすめは対象外。愛媛向けのみ)"
        "スポット(店・施設・観光地)を漏れなく抽出してください。\n"
        "- おすすめリストは本文ではなく添付画像内に書かれていることが多い。画像を必ず読むこと。\n"
        "- 【最重要】本文または画像に実際に名前が記載されているスポットのみを抽出すること。"
        "「松山のおすすめ」という文脈から定番観光地を推測・補完することは厳禁。"
...
# 愛媛県本体のおよその範囲 (外れたら out_of_pref。マージン付き bbox は pipeline.json)
PREF_STRICT = {"lat_min": 32.85, "lat_max": 34.35, "lng_min": 132.0, "lng_max": 133.75}
